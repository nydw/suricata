 ...
 *
 *  One of the utilities called by this function AppLayerHandleTCPData(), //lgx_mark StreamTcpReassembleInlineAppLayer
 ...
 *
 *  One of the utilities call by this function AppLayerHandleTCPData(),  // lgx_mark StreamTcpReassembleAppLayer
 *  has a feature where it will call this very same function for the
 *  stream opposing the stream it is called with.  This shouldn't cause
 *  any issues, since processing of each stream is independent of the
 *  other stream.
                                     ...
                                     Packet *p, PacketQueue *pq)
{
    ...
    if (StreamTcpInlineMode()) {
        int r = 0;
        if (StreamTcpReassembleInlineAppLayer(tv, ra_ctx, ssn, stream, p) < 0)  // lgx_mark
            r = -1;
...
static int StreamTcpReassembleStreamTest(TcpStream *stream) {
    ...
    p->payload = payload;
    p->payload_len = 3;
    if (StreamTcpReassembleHandleSegment(&tv, ra_ctx,&ssn, stream, p, &pq) == -1) {  // test
        SCFree(p);
        return 0;
    }
...
static int StreamTcpReassembleTest46 (void) {
    ...
    if (StreamTcpReassembleHandleSegment(&tv, ra_ctx, &ssn, s, p, &pq) == -1) { // test
        printf("failed in segments reassembly, while processing toserver packet\n");
        goto end;
    }
