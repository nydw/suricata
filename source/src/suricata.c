...
void GlobalInits()
{
    ...
    CreateLowercaseTable();  // 大写字母转小写字母表
}
...
TmEcode LoadYamlConfig(char *conf_filename) {
    ...
    if (includes != NULL) {
        TAILQ_FOREACH(file, &includes->head, next) {
            ...
            if (ConfYamlLoadFile(ifile) != 0) { // 解析include嵌套
                /* Error already displayed. */
                SCReturnInt(TM_ECODE_FAILED);
            }
        }
...
{
    if (SigLoadSignatures(de_ctx, suri->sig_file, suri->sig_file_exclusive) < 0) { // lgx_mark 载入所有的策略
        if (suri->sig_file == NULL) {
...
static int PostConfLoadedSetup(SCInstance *suri)
{
    ...
    MpmTableSetup();  // lgx_mark 设置多模式匹配表
#ifdef __SC_CUDA_SUPPORT__
    ...
    AppLayerDetectProtoThreadInit();  //lgx_mark 应用层检测初始化
    AppLayerParsersInitPostProcess(); //lgx_mark 建立了一个解析器之间的映射

    /* Check for the existance of the default logging directory which we pick
     * from suricata.yaml.  If not found, shut the engine down */
    suri->log_dir = ConfigGetLogDirectory(); // 设定日志存储目录 /var/log/suricata
    ...
    if (ConfigGetCaptureValue(suri) != TM_ECODE_OK) { // 获取与包捕获相关的一些配置参数，目前包括：max-pending-packets、default-packet-size。
        SCReturnInt(TM_ECODE_FAILED);
    }

    if (ConfGet("host-mode", &hostmode) == 1) {  // 设置host_mode（主机模式），两种模式：router和sniffer-only
    ...
    SCHInfoLoadFromConfig();  //lgx_mark 从配置文件中载入host os policy(主机OS策略)信息
    ...
    SigTableSetup(); // 主要是注册检测引擎所支持的规则格式中的关键字，比如sid、priority、msg、within、distance等等。
    TmqhSetup();     // lgx_mark 注册多线程调度模块的队列处理函数

    StorageInit();   // 初始化存储模块，这个模块可以用来临时存储一些数据.
    CIDRInit();      // 初始化CIDR掩码数组，cidrs[i]对应前i位为1的掩码.
    SigParsePrepare();  // lgx_mark 预处理pcre
...
#ifdef PROFILING                  // lgx_mark 几个Profiling模块的初始化函数。
    SCProfilingRulesGlobalInit(); // Profiling模块提供内建的模块性能分析功能，可以用来分析模块性能、各种锁的实际使用情况（竞争时间）、规则的性能等。
    ...
    RegisterAllModules(); //lgx_mark 注册了Suricata所支持的所有线程模块

    AppLayerHtpNeedFileInspection();

    DetectEngineRegisterAppInspectionEngines();

    if (suri->rule_reload) { // lgx_mark 规则热重启
        if (suri->sig_file != NULL)
    ...
    StorageFinalize();  // lgx_mark 关闭storage模块的注册，为已注册的storage实际分配存储空间。

    TmModuleRunInit();  // lgx_mark 调用之前注册的线程模块的初始化函数进行初始化。
...
{
    SCInstance suri; // lgx_mark 保存程序当前的一些状态、标志等上下文环境，通常是用来作为参数传递给各个模块

    SCInstanceInit(&suri);

    sc_set_caps = FALSE;  // 标识是否对主线程进行特权去除

    SC_ATOMIC_INIT(engine_stage);  // 记录程序当前的运行阶段 : SURICATA_INIT、SURICATA_RUNTIME、SURICATA_FINALIZE

    /* initialize the logging subsys */
    SCLogInitLogModule(NULL);  //lgx_mark 初始化日志模块
    ...
    SET_ENGINE_MODE_IDS(engine_mode);  //默认IDS, 而在nfq或ipfw启用时，就会切换成IPS模式
    ...
    ConfInit();  //lgx_mark 初始化配置模块

    if (ParseCommandLine(argc, argv, &suri) != TM_ECODE_OK) {
        exit(EXIT_FAILURE);
    }

    switch (StartInternalRunMode(&suri, argc, argv))  // lgx_mark 内部模式运行,运行结束退出, 不做检测
    {
    case TM_ECODE_DONE:
        exit(EXIT_SUCCESS);
    ...
    if (FinalizeRunMode(&suri, argv) != TM_ECODE_OK) {  // 运行非检测模式
        exit(EXIT_FAILURE);
    }

    if (suri.run_mode == RUNMODE_UNITTEST)  // 单元测试模式
    ...
    if (!CheckValidDaemonModes(suri.daemon, suri.run_mode)) // 检查当前模式是否与daemon标志冲突。Pcap文件模式及单元测试模式都不能在daemon开启下进行
    {
        exit(EXIT_FAILURE);
    }

    /* Initializations for global vars, queues, etc (memsets, mutex init..) */
    GlobalInits(); // lgx_mark 初始化全局变量
    TimeInit();    // lgx_mark 初始化时间
    SupportFastPatternForSigMatchTypes();  //lgx_mark 为快速模式匹配注册关键字
    ...
    if (LoadYamlConfig(conf_filename) != TM_ECODE_OK) {  // 加载配置
        exit(EXIT_FAILURE);
    }

    /* Since our config is now loaded we can finish configurating the
     * logging module. */
    SCLogLoadConfig(suri.daemon, suri.verbose); //lgx_mark 再次初始化日志模块。

    SCPrintVersion();  // 打印版本信息。

    UtilCpuPrintSummary();

    if (suri.run_mode == RUNMODE_DUMP_CONFIG) {
        ConfDump(); //lgx_mark 打印出当前的所有配置信息
        exit(EXIT_SUCCESS);
    }
    ...
    PacketPoolInit(max_pending_packets);  //lgx_mark 初始化报文池, 创建ringbuffer缓冲区
    HostInitConfig(HOST_VERBOSE);
    if (suri.run_mode != RUNMODE_UNIX_SOCKET) {
        FlowInitConfig(FLOW_VERBOSE);  //lgx_mark 初始化流表
    }
    ...
    if (!suri.delayed_detect) {
        if (LoadSignatures(de_ctx, &suri) != TM_ECODE_OK)  // lgx_mark 载入规则集
            exit(EXIT_FAILURE);
        if (suri.run_mode == RUNMODE_ENGINE_ANALYSIS) {
            exit(EXIT_SUCCESS);
        }
    ...
    CoredumpLoadConfig();   //lgx_mark 处理CoreDump相关配置
    ...
    if (suri.run_mode != RUNMODE_UNIX_SOCKET) {
        RunModeInitializeOutputs();  // 初始化所有Output模块。
    }

    if (ParseInterfacesList(suri.run_mode, suri.pcap_dev) != TM_ECODE_OK) {  // 解析配置文件中指定的Interface，并调用LiveRegisterDevice对其进行注册。
        exit(EXIT_FAILURE);
    }
    ...
    RunModeDispatch(suri.run_mode, suri.runmode_custom_mode, de_ctx); //lgx_mark 根据运行模式创建不同的线程
    ...
    if (suri.run_mode != RUNMODE_UNIX_SOCKET) {
        ...
        if (ConfGetBool("unix-command.enabled", &unix_socket) != 1)
            unix_socket = 0;
        if (unix_socket == 1) {
            UnixManagerThreadSpawn(de_ctx, 0);  //lgx_mark 创建命令通信线程
#ifdef BUILD_UNIX_SOCKET
        ...
        FlowManagerThreadSpawn();   //lgx_mark 会话管理线程 , 对流表进行超时删除处理
        StreamTcpInitConfig(STREAM_VERBOSE);
        SCPerfSpawnThreads(); 	//lgx_mark 创建性能计数相关线程
    }
