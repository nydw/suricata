...
AlpProtoDetectCtx alp_proto_ctx; // lgx_mark 保存了每个协议的特征码、协议号等。
                        ...
                        uint16_t offset, uint8_t flags, uint8_t ci)
{
    ...
    {
        al_proto_table[al_proto].name = name;  // lgx_mark 协议名称赋值
    }

    DetectContentData *cd = DetectContentParseEncloseQuotes(content); // lgx_mark 给特征字符串加双引号
    ...
    AlpProtoAddSignature(ctx, cd, ip_proto, al_proto);  // lgx_mark 加入模块特征链表
}
...
void AppLayerDetectProtoThreadInit(void)  // lgx_mark 初始化应用层协议检测模块
{
    AlpProtoInit(&alp_proto_ctx); // 初始化该模块协议特征多模式匹配器
    RegisterAppLayerParsers();   // 注册应用协议解析函数
    AlpProtoFinalizeGlobal(&alp_proto_ctx); // 调用匹配器的预处理（Prepare）函数、建立模式ID和规则签名之间的映射
        ...
        uint8_t flags, uint8_t ipproto)
{
    ...
    if (flags & STREAM_TOSERVER)
    {
        ...
        if (pp_port == NULL)
        {
            SCLogDebug("toserver-No probing parser registered for port %"PRIu16, f->dp);
    ...
    else
    {
        ...
        if (pp_port == NULL)
        {
            SCLogDebug("toclient-No probing parser registered for port %"PRIu16, f->sp);
            FLOW_SET_PP_DONE(f, flags);
            return ALPROTO_UNKNOWN;
        }
