...
void TmqhFlowRegister(void)
{
    ...
    tmqh_table[TMQH_FLOW].OutHandlerCtxSetup = TmqhOutputFlowSetupCtx; //autofp独有, 使用"flow"作为outqh的线程都会有多个输出队列,所以使用TmqhOutputFlowSetupCtx
    ...
    if (ConfGet("autofp-scheduler", &scheduler) == 1)
    {
        if (strcasecmp(scheduler, "round-robin") == 0)
        {
            ...
            tmqh_table[TMQH_FLOW].OutHandler = TmqhOutputFlowRoundRobin;  // 轮转调度：按照轮转方式依次选择下一个队列
        }
        else if (strcasecmp(scheduler, "active-packets") == 0)
        {
            SCLogInfo("AutoFP mode using \"Active Packets\" flow load balancer");
            tmqh_table[TMQH_FLOW].OutHandler = TmqhOutputFlowActivePackets;  // 最短匹配 ：选择当前最短（数据包最少）的队列
        }
        else if (strcasecmp(scheduler, "hash") == 0)
        {
            SCLogInfo("AutoFP mode using \"Hash\" flow load balancer");
            tmqh_table[TMQH_FLOW].OutHandler = TmqhOutputFlowHash;   // 最短匹配 ：选择当前最短（数据包最少）的队列
        }
        else
        {
...
void *TmqhOutputFlowSetupCtx(char *queue_str)
{
    ...
    TmqhFlowCtx *ctx = SCMalloc(sizeof(TmqhFlowCtx));  /// 该结构体将被存储在ThreadVars的outctx中
    ...
    do
    {
        ...
        if (comma != NULL)
        {
            *comma = '\0';
            char *qname = tstr;
            int r = StoreQueueId(ctx,qname);  // 将每个qname传给StoreQueueId，以创建相应名字队列并存储到ctx中
            if (r < 0)
                goto error;
        }
        else
