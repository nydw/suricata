...
void TmModuleReceivePcapRegister (void)
{
    ...
    tmm_modules[TMM_RECEIVEPCAP].PktAcqLoop = ReceivePcapLoop; // 抓包函数
...
TmEcode ReceivePcapLoop(ThreadVars *tv, void *data, void *slot)
{
    ...
    while (1) {
        ...
        r = pcap_dispatch(ptv->pcap_handle, (int)packet_q_len,  // lgx_mark 无限循环调用pcap_dispatch接口取包
 ...
 */
#if LIBPCAP_VERSION_MAJOR == 1
TmEcode ReceivePcapThreadInit(ThreadVars *tv, void *initdata, void **data) {  // data 初始化函数的结果输出
    SCEnter();
 ...
 */
TmEcode DecodePcap(ThreadVars *tv, Packet *p, void *data, PacketQueue *pq, PacketQueue *postpq) // data为初始化时填充的DecodeThreadVars，pq为解码模块所嵌入的slot的slot_pre_pq，postpq则为slot_post_pq（可能为NULL）。
{
    SCEnter();
...
TmEcode DecodePcapThreadInit(ThreadVars *tv, void *initdata, void **data)   // lgx_mark 模块初始化
{
    SCEnter();
