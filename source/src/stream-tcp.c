...
void TmModuleStreamTcpRegister (void)
{
    ...
    tmm_modules[TMM_STREAMTCP].Func = StreamTcp;  // lgx_mark
...
void StreamTcpInitConfig(char quiet)  // lgx_mark 完成StreamTCP的配置初始化
{
    intmax_t value = 0;
    uint16_t rdrange = 10;
    ...
    StreamTcpReassembleInit(quiet);  //lgx_mark 重组模块初始化
...
int StreamTcpPacket (ThreadVars *tv, Packet *p, StreamTcpThread *stt,  // lgx_mark
                     PacketQueue *pq)
{
    SCEnter();
    ...
    else
    {
        ...
        switch (ssn->state) {  // lgx_mark state machine
        case TCP_SYN_SENT:
...
TmEcode StreamTcp (ThreadVars *tv, Packet *p, void *data, PacketQueue *pq, PacketQueue *postpq)
{
    ...
    ret = StreamTcpPacket(tv, p, stt, pq);  // lgx_mark    解析TCP状态机
...
static int StreamTcpTest02 (void) {
    ...
    SCMutexLock(&f.m);
    if (StreamTcpPacket(&tv, p, &stt, &pq) == -1) {   //  test
        goto end;
    }
...
static int StreamTcpTest23(void)
{
    ...
    if (StreamTcpReassembleHandleSegment(&tv, ra_ctx,&ssn, &ssn.client, p, &pq) == -1) {  // test
