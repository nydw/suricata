...
void TmModuleReceivePcapFileRegister (void) {
    ...
    tmm_modules[TMM_RECEIVEPCAPFILE].PktAcqLoop = ReceivePcapFileLoop;  // lgx_mark
...
void PcapFileCallbackLoop(char *user, struct pcap_pkthdr *h, u_char *pkt) {
    ...
    if (TmThreadsSlotProcessPkt(ptv->tv, ptv->slot, p) != TM_ECODE_OK)  // // lgx_mark 进一步的数据包处理
    {
...
TmEcode ReceivePcapFileLoop(ThreadVars *tv, void *data, void *slot)
{
    ...
    while (1) {
        ...
        r = pcap_dispatch(pcap_g.pcap_handle, (int)packet_q_len,                  // lgx_mark pcap_file
...
//    SCPerfCounterIncr(dtv->counter_pkts_per_sec, tv->sc_perf_pca);
    ...
    /* call the decoder */
    pcap_g.Decoder(tv, dtv, p, GET_PKT_DATA(p), GET_PKT_LEN(p), pq);  // lgx_mark DecodeEthernet
