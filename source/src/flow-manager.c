...
uint32_t FlowTimeoutHash(struct timeval *ts, uint32_t try_cnt, FlowTimeoutCounters *counters)
{
    ...
    for (idx = 0; idx < flow_config.hash_size; idx++)
    {
        ...
        cnt += FlowManagerHashRowTimeout(fb->tail, ts, emergency, counters); //lgx_mark 对这个bucket上的flow链表（Row）进行超时
 ...
 */
void *FlowManagerThread(void *td)   //lgx_mark 用于对超时的流进行删除
{
    ...
    while (1)
    {
        ...
        FlowUpdateSpareFlows();  // 保证当前空闲流的数量正好等于prealloc，少的就alloc，多的就free.

        /* try to time out flows */
        FlowTimeoutCounters counters = { 0, 0, 0, };
        FlowTimeoutHash(&ts, 0 /* check all */, &counters);  // 进行实际的流超时处理
...
void FlowManagerThreadSpawn()
{
    ...
    tv_flowmgr = TmThreadCreateMgmtThread("FlowManagerThread",
                                          FlowManagerThread, 0);  // lgx_mark 创建流表超时删除处理线程
