...
/* thread flags */
#define TM_FLAG_RECEIVE_TM      0x01  // 收包模块标记ID
#define TM_FLAG_DECODE_TM       0x02  // 解码模块标记ID
#define TM_FLAG_STREAM_TM       0x04
#define TM_FLAG_DETECT_TM       0x08

typedef struct TmModule_ {   // lgx_mark 负责传参,初始化插槽字段
    char *name;
    ...
    TmEcode (*Func)(ThreadVars *, Packet *, void *, PacketQueue *, PacketQueue *);  // 包处理函数 DecodePcap etc, 没有就NULL

    TmEcode (*PktAcqLoop)(ThreadVars *, void *, void *);  // 收包函数
    ...
    uint8_t cap_flags;   // 标志这个线程模块所需要的能力，以确定能让整个系统正常运行所需要的最小权限
