...
void RunModeIdsPcapRegister(void) // lgx_mark 注册运行模式到全局变量 runmodes
{
    RunModeRegisterNewRunMode(RUNMODE_PCAP_DEV, "single",  // 单线程模式，不需要队列
                              "Single threaded pcap live mode",
                              RunModeIdsPcapSingle);
    RunModeRegisterNewRunMode(RUNMODE_PCAP_DEV, "auto",  // 多线程自动模式，同一个流的数据包可能被多个检测线程处理。
                              "Multi threaded pcap live mode",
                              RunModeIdsPcapAuto);
    default_mode = "autofp";
    RunModeRegisterNewRunMode(RUNMODE_PCAP_DEV, "autofp",  //多线程自动流绑定负载均衡模式（auto flow pinned load balancing），每个流的包只会被同一个检测线程处理（flow pinned）。其中pickup对立个数与其reader线程数量一致，因为每个线程只能有1个inq。
    ...
    RunModeRegisterNewRunMode(RUNMODE_PCAP_DEV, "workers",  //lgx_mark 多线程工人模式，但每个线程做所有事（从抓包到输出日志），因此也不需要队列
...
int RunModeIdsPcapAutoFp(DetectEngineCtx *de_ctx)
{
    ...
    ret = RunModeSetLiveCaptureAutoFp(de_ctx,  // lgx_mark
                              ParsePcapConfig,
                              PcapConfigGeThreadsCount,
                              "ReceivePcap",
                              "DecodePcap", "RxPcap",
