 ...
 */
int RunModeFilePcapSingle(DetectEngineCtx *de_ctx)  // RunModeDispatch 运行此函数启动线程
{
    char *file = NULL;
    ...
    tm_module = TmModuleGetByName("StreamTcp");  // lgx_mark
    if (tm_module == NULL) {
    ...
    tm_module = TmModuleGetByName("Detect");  //lgx_mark 特征检查模块
    if (tm_module == NULL) {
        SCLogError(SC_ERR_RUNMODE, "TmModuleGetByName Detect failed");
        exit(EXIT_FAILURE);
    }
    TmSlotSetFuncAppend(tv, tm_module, (void *)de_ctx);  // 添加到slot链表

    SetupOutputs(tv);  //lgx_mark 输出模块

    TmThreadSetCPU(tv, DETECT_CPU_SET);

    if (TmThreadSpawn(tv) != TM_ECODE_OK) {  //lgx_mark 创建新线程
...
int RunModeFilePcapAuto(DetectEngineCtx *de_ctx)
{
    ...
    int thread;
    for (thread = 0; thread < thread_max; thread++) {   // 创建detect线程
        ...
        if (TmThreadSpawn(tv_detect_ncpu) != TM_ECODE_OK) {  // 建立线程
            SCLogError(SC_ERR_RUNMODE, "TmThreadSpawn failed");
            exit(EXIT_FAILURE);
        }
...
int RunModeFilePcapAutoFp(DetectEngineCtx *de_ctx)
{
    ...
    TmModule *tm_module = TmModuleGetByName("ReceivePcapFile");// 接收
    if (tm_module == NULL) {
        SCLogError(SC_ERR_RUNMODE, "TmModuleGetByName failed for ReceivePcap");
        exit(EXIT_FAILURE);
    }
    ...
    tm_module = TmModuleGetByName("DecodePcapFile");  // 解码
    if (tm_module == NULL) {
        SCLogError(SC_ERR_RUNMODE, "TmModuleGetByName DecodePcap failed");
        exit(EXIT_FAILURE);
    }
    ...
    for (thread = 0; thread < thread_max; thread++)
    {
        ...
        tm_module = TmModuleGetByName("StreamTcp");  // stream管理
        if (tm_module == NULL) {
            SCLogError(SC_ERR_RUNMODE, "TmModuleGetByName StreamTcp failed");
            exit(EXIT_FAILURE);
        }
        ...
        tm_module = TmModuleGetByName("Detect");   // detect 检测
        if (tm_module == NULL) {
            SCLogError(SC_ERR_RUNMODE, "TmModuleGetByName Detect failed");
            exit(EXIT_FAILURE);
        }
