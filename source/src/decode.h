...
typedef struct Packet_
{
    ...
    /* flow */
    uint8_t flowflags;   // FLOW_PKT_ESTABLISHED
    /* coccinelle: Packet:flowflags:FLOW_PKT_ */

    /* Pkt Flags */
    uint32_t flags;  // PKT_IGNORE_CHECKSUM  ，表示后续不再对其进行校验和验证
    ...
    uint8_t pkt_src;  // 数据包源（PKT_SRC_WIRE）
...
typedef struct DecodeThreadVars_
{
    ...
    AlpProtoDetectThreadCtx udp_dp_ctx;   // lgx_mark 用于UDP包的应用层协议检测
    ...
    uint16_t counter_ipv4;   // 统计IPV4数据包个数
