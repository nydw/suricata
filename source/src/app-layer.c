...
uint16_t AppLayerGetProtoFromPacket(Packet *p)  // 返回应用层协议
{
    SCEnter();
...
void *AppLayerGetProtoStateFromPacket(Packet *p)  // 获得协议状态
{
    SCEnter();
                          ...
                          uint8_t flags)
{
    ...
    if (*alproto == ALPROTO_UNKNOWN && (flags & STREAM_GAP)) {
        ...
        *alproto = AppLayerDetectGetProto(&alp_proto_ctx, dp_ctx, f,  // lgx_mark  tcp
        ...
        if (f->alproto != ALPROTO_UNKNOWN)
        {
            ...
            r = AppLayerParse(dp_ctx->alproto_local_storage[f->alproto], f, f->alproto, flags, data, data_len); // lgx_mark 应用协议解析
...
int AppLayerHandleUdp(AlpProtoDetectThreadCtx *dp_ctx, Flow *f, Packet *p)
{
    ...
    if (f->alproto == ALPROTO_UNKNOWN && !(f->flags & FLOW_ALPROTO_DETECT_DONE)) {
        ...
        PACKET_PROFILING_APP_PD_START(dp_ctx);
        f->alproto = AppLayerDetectGetProto(&alp_proto_ctx, dp_ctx, f,  // lgx_mark udp
