 ...
 */
PacketQueue trans_q[256];  // lgx_mark 线程间队列

SCDQDataQueue data_queues[256];

typedef struct SCInstance_ {    // lgx_mark 保存程序当前的一些状态、标志等上下文环境，通常是用来作为参数传递给各个模块
    int run_mode;  // 抓包模式
    ...
    char *keyword_info;
    char *runmode_custom_mode;  // autofp
#ifndef OS_WIN32
    char *user_name;
    char *group_name;
    uint8_t do_setuid;
    ...
    struct timeval start_time;  // 系统的启动时间
