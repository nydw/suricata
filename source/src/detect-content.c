                           ...
                           uint8_t **pstr, uint16_t *plen, uint32_t *flags)
{
    ...
    if ((str = SCStrdup(contentstr + pos + 1)) == NULL) // 去掉头部 "
        goto error;
    str[strlen(str) - 1] = '\0';  // 去掉尾部 "
...
DetectContentData *DetectContentParse (char *contentstr)
{
    ...
    cd->bm_ctx = BoyerMooreCtxInit(cd->content, cd->content_len);  // lgx_mark 特征码BM
    cd->depth = 0;
    cd->offset = 0;
    cd->within = 0;
    cd->distance = 0;
