...
typedef struct TcpStreamCnf_ {
     ...
     */
    uint64_t memcap;            // stream跟踪功能所能使用的最大内存，默认为32MB
    uint64_t reassembly_memcap; // 重组功能所能使用的最大内存，默认为64MB.

    uint32_t ssn_init_flags;    // 新session标志
    uint8_t segment_init_flags; // 新segment标志

    uint32_t prealloc_sessions; // stream跟踪功能所能使用的最大内存，默认为32MB
    int midstream;              // 是否允许接手midstream session，默认为关闭。
    int async_oneside;          // 是否打开异步stream处理，默认为关闭。
    uint32_t reassembly_depth;  // 重组深度
