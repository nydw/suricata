...
#define FLOW_PKT_ESTABLISHED            0x04  // 若流的FLOW_TO_DST_SEEN和FLOW_TO_SRC_SEEN都设置了，说明流已经建立，给包添加此标志
...
{
    uint32_t hash_rand;  // 用于随机化hash table的分布
    uint32_t hash_size;  // 流表所使用的hash table的大小  默认为65536
    uint64_t memcap;     // flow engine所允许使用的最大内存数量  默认为32MB
    uint32_t max_flows;
    uint32_t prealloc;  // 预分配的流的数量  默认为10000
    ...
    uint32_t emergency_recovery;  // 退出紧急状态前需要删除的流的百分比，默认为30
...
typedef struct Flow_
{
    ...
    SC_ATOMIC_DECLARE(int, autofp_tmqh_flow_qid);  // 记录这个流所被绑定到的输出队列的ID

    uint32_t probing_parser_toserver_al_proto_masks;
    uint32_t probing_parser_toclient_al_proto_masks;

    uint32_t flags;  // FLOW_TO_DST_SEEN 表示发现了至少一个发往服务端的数据包

    /* ts of flow init and last update */
    int32_t lastts_sec; // 最近一个包接收的时间,用于流超时.
 ...
 */
static inline void FlowIncrUsecnt(Flow *f) // 增加该流的计数
{
    if (f == NULL) return;
