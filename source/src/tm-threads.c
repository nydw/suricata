...
ThreadVars *tv_root[TVT_MAX] = { NULL };  // lgx_mark
 ...
 */
int TmThreadsCheckFlag(ThreadVars *tv, uint16_t flag) // lgx_mark  等待子线程初始化完成
{
...
void *TmThreadsSlot1(void *td)
{
    ...
    while (run)
    {
        ...
        /* input a packet */
        p = tv->tmqh_in(tv);  // lgx_mark 取一个数据包
                            ...
                            TmSlot *slot)
{
    ...
    for (s = slot; s != NULL; s = s->slot_next)
    {
        ...
        if (unlikely(s->id == 0))
        {
            r = SlotFunc(tv, p, SC_ATOMIC_GET(s->slot_data), &s->slot_pre_pq, &s->slot_post_pq);  // tm->Func
        }
        else
        {
...
void *TmThreadsSlotPktAcqLoop(void *td) {
    ...
    while(run)
    {
        ...
        if (r == TM_ECODE_FAILED || TmThreadsCheckFlag(tv, THV_KILL) || suricata_ctl_flags)
        {
            run = 0;
        }
...
TmEcode TmThreadSetSlots(ThreadVars *tv, char *name, void *(*fn_p)(void *))
{
    ...
    } else if (strcmp(name, "varslot") == 0) {   // autofp
        tv->tm_func = TmThreadsSlotVar;
    } else if (strcmp(name, "pktacqloop") == 0) {  // lgx_mark
...
static inline TmSlot * _TmSlotSetFuncAppend(ThreadVars *tv, TmModule *tm, void *data)
{
    ...
    (void)SC_ATOMIC_SET(slot->SlotFunc, tm->Func); // lgx_mark
    slot->PktAcqLoop = tm->PktAcqLoop;  // lgx_mark
 ...
 */
ThreadVars *TmThreadCreate(char *name, char *inq_name, char *inqh_name, // lgx_mark 创建新线程变量
                           ...
                           void * (*fn_p)(void *), int mucond)
{
    ...
    if (inq_name != NULL && strcmp(inq_name, "packetpool") != 0) {
        ...
        if (tmq == NULL) {
            tmq = TmqCreateQueue(inq_name); // lgx_mark 创建输入队列
            if (tmq == NULL)
                goto error;
        }
    ...
    if (inqh_name != NULL) {
        ...
        tv->tmqh_in = tmqh->InHandler;  // lgx_mark
    ...
    if (outqh_name != NULL) {
        ...
        tv->tmqh_out = tmqh->OutHandler;  // lgx_mark
        ...
        if (outq_name != NULL && strcmp(outq_name, "packetpool") != 0)
        {
            ...
            else
            {
                ...
                if (tmq == NULL)
                {
                    tmq = TmqCreateQueue(outq_name);  // lgx_mark 创建输出队列
                    if (tmq == NULL)
                        goto error;
                }
                                        ...
                                        char *outqh_name, char *slots)
{
    ...
    tv = TmThreadCreate(name, inq_name, inqh_name, outq_name, outqh_name,  // lgx_mark
                        slots, NULL, 0);
 ...
 */
TmEcode TmThreadSpawn(ThreadVars *tv)   // lgx_mark 创建线程
{
    pthread_attr_t attr;
    if (tv->tm_func == NULL) {
    ...
    int rc = pthread_create(&tv->t, &attr, tv->tm_func, (void *)tv);  // 创建线程   TmThreadsSlotPktAcqLoop
    if (rc) {
